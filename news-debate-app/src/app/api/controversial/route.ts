...
const AI_PROMPT = `Analyze the political bias of the following Israeli news entities (right, center, left). Return ONLY a JSON object. Entities: `;
...
export async function GET() {
  try {
    ...
    // 3. חילוץ שמות לסיווג
    const entities = new Set<string>(sourcesRes.sources?.map((s: any) => s.name) || []);
    allArticles.forEach(a => {
      if (a.sourceName && !['Telegram', 'NewsAPI'].includes(a.sourceName)) entities.add(a.sourceName);
    ...
    // 5. כתיבת מפתח בדיקה והחזרת תשובה
    ...
    return NextResponse.json({ newsItems: finalNews.slice(0, 30) });
